...
if __name__ == "__main__":
    ...
    v2ray_parser = sub_parsers.add_parser("v2ray", help="Fill input file or dir with env")
    v2ray_parser.set_defaults(func=v2ray)
    v2ray_parser.add_argument('--json-dir', help="input group of configs as a dir", action='store', type=str, required=False)
    v2ray_parser.add_argument('--json-file', help="input a config file", action='store', type=str, required=False)
    v2ray_parser.add_argument('--output-dir', help="path to the output dir", action='store', type=str, required = True)
    
    # nginx config subparser
    nginx_parser = sub_parsers.add_parser("nginx", help="conver nginx conf file to json and vice versa")
    nginx_parser.set_defaults(func=nginx)
    nginx_parser.add_argument('--input-file-conf', help="path to the input conf file", action='store', type=str, required=False)
    nginx_parser.add_argument('--output-file-conf', help="path to the output conf file", action='store', type=str, required=False)
    nginx_parser.add_argument('--input-file-json', help="path to the input json file", action='store', type=str, required=False)
    nginx_parser.add_argument('--output-file-json', help="path to the output json file", action='store', type=str, required=False)
    
    # backup config subparser
    backup_parser = sub_parsers.add_parser("backup", help="Get backup from sword fish env and other files")
    backup_parser.set_defaults(func=backup)
    backup_parser.add_argument('--device', help="type of device to use", choices=['middle', 'end'], required=True)
    backup_parser.add_argument('--output', help="path to the output folder", action='store', type=str, required=False, default="./")
    backup_parser.add_argument('--passwd', help="password to encrypt the backup file", action='store', type=str, required=True)
    
    # setup config subparser
    setup_parser = sub_parsers.add_parser("setup", help="build the necessary configs with the given env files")
    setup_parser.set_defaults(func=setup)
    setup_parser.add_argument('--device', help="type of device to use", choices=['middle', 'end'], required=True)
    setup_parser.add_argument('--gen-cert', help="genrate the necessary certificates for the sword fish", action='store_true', required=False)
    setup_parser.add_argument('--nginx', help="change host nginx configs", action='store_true', required=False)
    
    # monitor config subparser
    monitor_parser = sub_parsers.add_parser("monitor", help="start and stop monitor server")
    monitor_parser.set_defaults(func=monitor)
    monitor_parser.add_argument('--start', help="start monitor server in background", action='store_true', required=False)
    monitor_parser.add_argument('--terminate', help="terminate the monitor server", action='store_true', required=False)
    monitor_parser.add_argument('--watch', help="do not start server as a background task", action='store_true', required=False)
    monitor_parser.add_argument('--host', help="host ip for setting up the monitor server", action='store', type=str, required=False, default="127.0.0.1")
    monitor_parser.add_argument('--port', help="port for setting up the monitor server", action='store', type=int, required=False, default=8000)
    
    # setupb config subparser
    setupb_parser = sub_parsers.add_parser("setupb", help="fill and env and other configs with the given backup")
    setupb_parser.set_defaults(func=setupb)
    setupb_parser.add_argument('--device', help="type of device to use", choices=['middle', 'end'], required=True)
    setupb_parser.add_argument('--from-input', help="path to the backup file", action='store', required=True)
    setupb_parser.add_argument('--passwd', help="password for decrypting the backup file", action='store', type=str, required=True)
